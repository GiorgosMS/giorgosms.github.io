jQuery.fn.highlight = function (c, f) {
    var b = "";
    if (f) {
        b = a(c)
    } else {
    ...
    function d(g, k) {
        ...
        if (g.nodeType == 3) {
            ...
            if (f) {
                ...
                j = g.data.toUpperCase().indexOf(k)
            }
            if (j >= 0) {
    ...
    function a(g) {
        ...
        var j = k.replace(/([|()[{.+*?^$\\])/g, "\\$1");
        var h = function (l) {
            return charToAccentedCharClassMap[l] || l
        };
    ...
    function e(h) {
        var g = h;
        for (var i in charToAccentedCharClassMap) {
            g = g.replace(new RegExp(charToAccentedCharClassMap[i], "g"), i)
        }
        return g
    }
};
